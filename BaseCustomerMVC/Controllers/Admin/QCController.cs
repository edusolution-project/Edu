...
namespace BaseCustomerMVC.Controllers.Admin
{
    ...
    public class QCController : AdminController
    {
        private readonly QCService _servicesAds;
        private readonly IHostingEnvironment _evn;
        private readonly FileProcess _fileProcess;

        public QCController(QCService servicesAds, IHostingEnvironment evn,
        ...
        public async Task<JsonResult> CreateCampaign(QCEntity camp, IFormFile Banner)
        {
            if (string.IsNullOrEmpty(camp.ID) || camp.ID == "0")
            {
                ...
                if (Banner != null)
                {
                    camp.Banner = await _fileProcess.SaveMediaAsync(Banner, "", "QC");
                    //urlBanner(Banner);
                }
        ...
        public JsonResult getAllCampaign(DefaultModel model)
        {
            ...
            {
                filter.Add(Builders<QCEntity>.Filter.Text(model.SearchText));
            }
            var data = (filter.Count > 0 ? _servicesAds.Collection.Find(Builders<QCEntity>.Filter.And(filter)) : _servicesAds.GetAll())
