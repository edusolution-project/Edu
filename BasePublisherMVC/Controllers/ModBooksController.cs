...
namespace BasePublisherMVC.AdminControllers
{
    [MenuControl(
        CModule = "ModBooks",
        Name = "MO : Quản lý sách",
        Order = 40,
        IShow = true,
        Type = MenuType.Mod
    )]
    public class ModBooksController : AdminController
    {
        private readonly ModBookService _service;
        public ModBooksController(ModBookService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(DefaultModel model, ModBookEntity item)
        {
            ViewBag.Title = "Thêm mới";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(DefaultModel model, ModBookEntity item)
        {
            ViewBag.Title = "Chỉnh sửa";
            if (string.IsNullOrEmpty(model.ID) && string.IsNullOrEmpty(item.ID))
            {
