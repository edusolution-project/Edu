...
namespace BasePublisherMVC.AdminControllers
{
    ...
    {
        private readonly ModUnitService _service;
        public ModUnitsController(ModUnitService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(DefaultModel model, ModUnitEntity item)
        {
            ViewBag.Title = "Thêm mới";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(DefaultModel model, ModUnitEntity item)
        {
            ViewBag.Title = "Chỉnh sửa";
            if (string.IsNullOrEmpty(model.ID) && string.IsNullOrEmpty(item.ID))
            {
