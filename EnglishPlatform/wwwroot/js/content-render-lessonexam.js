...
var Lesson = (function () {
    ...
    var loadLessonData = function (param, callback) {
        ...
        Object.keys(param).forEach(e => console.log(e + param[e]));
        Ajax(config.url.load, formData, "POST", true).then(function (res) {
            if (!isNull(res)) {
                ...
                switch (config.mod) {
                    ...
                    case mod.TEACHERVIEW:
                    case mod.TEACHEREDIT:
                        loadLessonParts({ LessonExamID: config.lessonexam_id, ClassID: config.class_id, ClassSubjectID: config.class_subject_id }, callback);
                        break;
                    case mod.STUDENT_REVIEW:
                        loadLessonParts({ LessonExamID: config.lessonexam_id, ClassID: config.class_id, ClassSubjectID: config.class_subject_id, ExamID: config.exam_id }, callback);
                        break;
                    default:
                        callback();
                        break;
    ...
    var loadLessonParts = function (param, callback) {
        console.log(param + " loadLessonParts - line 262");
        var formData = new FormData();
        if (param != null)
            Object.keys(param).forEach(e => formData.append(e, param[e]));
        debugger
    ...
    var renderExamLayout = function () {
        var container = $('#' + config.container);
        container.addClass("text-center bg-secondary");
        container.empty();

        var lessonBox = $("<div>", { "class": "lesson lesson-box d-inline-block text-left h-100 mod_" + config.mod });
        ...
        var lessonContent = $("<div>", { "class": "card border-0 h-100 rounded-0" });
        lessonContainer.append(lessonContent);

        var headerBox = $("<div>", { "class": "examHeader mb-2 d-flex flex-column justify-content-between p-3" });
        lessonContent.append(headerBox);
    ...
    var renderExamInfo = function () {
        ...
        switch (config.mod) {
            ...
            case mod.TEACHERVIEW:
            case mod.TEACHEREDIT:
                codeexam = $("<div>", { class: "col-6 text-center", html: "Mã đề: <span style='font-weight:700'>" + config.codeExam +"</span>" });
                break;
        }
    ...
    var renderLessonData = function () {
        ...
        switch (config.mod) {
            ...
            case mod.TEACHERVIEW:
                ...
                var previewMode = $("<button>", { "class": "btn btn-primary btn-add mt-2 mb-2 mr-2", "title": "Xem trước", "onclick": "openPreview()" });
                var iconPreview = $("<i>", { "class": "far fa-paper-plane mr-2" });
                previewMode.append(iconPreview).append("Xem trước");
            ...
            case mod.STUDENT_LECTURE:
                ...
                switch (_UImode) {
                    ...
                    case UIMode.BOTH:
                        ...
                        dataform.append("LessonExamID", config.lessonexam_id);
                        //debugger
            ...
            case "QUIZ2":
                ...
                for (var i = 0; data.Questions != null && i < data.Questions.length; i++) {
                    ...
                    try {
                        ...
                        var input = $("<span>", { class: "fillquiz quiz-item", id: item.ID, "data-part-id": item.ParentID });
    ...
    var goQuiz = function (quizid, obj, scroll = true) {
        ...
        if (part != null) {
            ...
            $('#PartNav .selection').removeClass('selection');
            ...
            if (obj != null)
                ...
                $('#quizNav' + quizid).addClass('selection');
    ...
    var renderPartNav = function (data) {
        if ($('#PartNav').length == 0) {
            $("#QuizNav").parent().append($('<div>', { id: 'PartNav', class: 'show' }));
            $("#PartNav").append($('<div>', { html: "<b style='font-weight: 700'>Cấu trúc bài<b>", style: 'border-bottom: solid 1px #CCC', class: 'mb-2 pb-2 text-right' }));
            $("#PartNav").append($('<div>', { id: 'part-total-point', 'data-point': 0, html: "<b style='font-weight: 700'>Tổng<b>", style: 'border-top: solid 1px #CCC', class: 'mt-2 pt-2 text-right' }));
        }
        ...
        var partNav = $('<div>', { id: 'part-nav-' + data.ID, text: 'Câu ' + ($('#PartNav').children().length - 1), class: 'mb-2 bg-white text-right', style: 'cursor:pointer', onclick: "window.GoQuiz(\'" + firstQuiz + "\', this)" })
            .append($('<span>', { id: 'part-point-' + data.ID, text: ' - ' + point + 'đ', style: 'font-weight: 700' }));
        $('#part-total-point').before(partNav);
    }
    ...
    var AnswerQuestion = function (_this, _that) {
        $('.btn-answer-idx.selection').removeClass('selection');
    ...
    var renderQuizCounter = function () {
        ...
        for (var i = 0; listQuiz != null && i < listQuiz.length; i++) {
            ...
            answerList += '<button class="btn btn-answer-idx text-white rounded-quiz ' + completed + '" type="button" id="quizNav' + item.id + '" name="quizNav' + item.id + '" onclick="window.GoQuiz(\'' + item.id + '\', this)">' + (i + 1) + '</button>';
        }
        ...
        $("#QuizNav").empty()
            .append($('<div>', { html: "<b style='font-weight: 700'>Danh sách câu hỏi<b>", style: 'border-bottom: solid 1px #CCC', class: 'mb-2 p-2' }))
            .append(quizNavigator);
    ...
    var fillquizFocus = function (obj) {
        var quizId = $(obj).attr('data-question-id');
        goQuiz(quizId, null, false);
        //$(obj).parent().addClass('selection');
        $(obj)[0].dataset.oldval = $(obj).text();
    }

    var fillquizBlur = function (obj) {
        //$(obj).parent().removeClass('selection');
        ...
        Ajax(config.url.load_classsubject, dataform, "POST", false)
            .then(function (res) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                    if (item.CourseName)
                        $(ulselectTemplate).append("<li style='padding:10px' id='" + item.ID + "' for='" + item.ID + "'><div style='font-size:14px;color:#00BBD4;font-weight:700'><span style='cursor:pointer' onclick='chooseCourse(\"" + item.ID + "\",this)'>" + item.CourseName + "<i class='far fa-arrow-alt-circle-down ml-1' style='color:" + item.Color + "' ></i></span></div></li>");
                }
            })
    }
    ...
    var chooseCourse = function (id, obj) { //chương
        ...
        var icon = $(obj).find("i");
        var classi = $(icon).attr("class");
        if (classi.includes("fa-arrow-alt-circle-down")) {
            $(icon).removeClass();
            $(icon).addClass("far fa-arrow-alt-circle-left ml-1");
                    ...
                    .then(function (res) {
                        ...
                        for (var i = 0; i < chapters.length; i++) {
                            ...
                            if (chapter.ParentID == "0") {
                                ...
                                //add chapter vào parent chapter
                                var containerSubChapter = $("#" + chapter.ParentID);
                                //var firstChilSub = containerSubChapter.children()[0];
                                //console.log(firstChilSub);
                                var ulselectSubChapterTemplate = containerSubChapter.find('ul#selectSubChapter');
                                if (ulselectSubChapterTemplate.length == 0) {
                                    ulselectSubChapterTemplate = $("<ul>", { "class": "list hide", "required": "required", "id": "selectSubChapter" });
                                    containerSubChapter.append(ulselectSubChapterTemplate);
                                }
                                //$("<ul>", { "class": "list hide", "required": "required", "id": "selectSubChapter" });
                            ...
                            var ulselectLessonTemplate = containerLesson.find('ul#selectLesson');
                            if (ulselectLessonTemplate.length == 0) {
                                ulselectLessonTemplate = $("<ul>", { "class": "list hide", "required": "required", "id": "selectLesson" });
            ...
            containerCourse.append(child);
            $(icon).removeClass();
            $(icon).addClass("far fa-arrow-alt-circle-down ml-1");
        }
    }

    var chooseLesson = function (id, objHolder) { //bài trong chương
        //debugger
    ...
    var choosePart = function (lessonid, classsbjid, objHolder) {
        console.log("choosePart - line 4973 - LessonID" + lessonid)
        //var modalForm = window.partModaltoAddExam;
