...
CKEDITOR.config.specialChars = CKEDITOR.config.specialChars.concat([['&uarr;', 'Mũi tên hướng lên trên'], ['&darr;', 'Mũi tên hướng xuống dưới']]);
...
var Lesson = (function () {
            ...
            if (isContinue) {
                ...
                lastExamResult =
                    $("<div>", { id: "last-result", class: "text-center" })
                        ...
                        .append($('<div>', { class: "col-md-12 text-center h4 text-success pb-3", text: "Kết quả: " + lastExam.Point + "/" + lastExam.MaxPoint }));//.html();


                //console.log(progress);
                if (learningTarget > 0) {
                    if (progress.toFixed(0) >= learningTarget) {
                        resultMessage = "Chúc mừng bạn đã hoàn thành mục tiêu (" + (progress / 10).toFixed(1) + "/" + (learningTarget / 10) + ")";
                        ...
                        resultMessage = "Bạn đã gần đạt mục tiêu rồi (" + (progress / 10).toFixed(1) + "/" + (learningTarget / 10) + "). Cố lên!";
                        lastExamResult.append($('<div>', { class: "col-md-12 text-center h4 pb-3 text-warning", text: resultMessage }));//.html();
                    }
                    else {
                        resultMessage = "Bạn chưa đạt điểm mục tiêu đề ra: (" + (progress / 10).toFixed(1) + "/" + (learningTarget / 10) + ").<br/> Bạn cần cố gắng nhiều hơn nhé!";
    ...
    var chooseLesson = function (id, obj) { //bài trong chương
        ...
        var classi = $($(obj).children()[1]).attr('class');
        if (classi.includes("fa-arrow-alt-circle-down")) {
            $($(obj).children()[1]).removeClass();
            $($(obj).children()[1]).addClass("far fa-arrow-alt-circle-left ml-1");
        }
        else {
            $($(obj).children()[1]).removeClass();
            $($(obj).children()[1]).addClass("far fa-arrow-alt-circle-down ml-1");
        }
    ...
    var choosePart = function (lessonid, classsbjid, obj) {
        var classUl = $($(obj).children()[1]).attr("class");
    ...
    var ShowAddPartToLesson = function (id, type) {
        id = "'" + id + "'";
        type = "'" + type + "'";
        if (config.mod == mod.TEACHEREDIT) { //soạn thảo
            Swal.fire({
                title: '<strong>Chọn thao tác</strong>',
                icon: 'question',
                html:
