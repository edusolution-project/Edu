...
function toggleAnnouncement(obj, _id) {
    var type = 1;//anouncement
    if ($(obj).hasClass("active")) {
        type = 0;//normal
    }
    $.ajax({
        type: "POST",
        url: _ToggleFeedTypeURL,
        data: { ID: _id, FeedType: type },
        dataType: "json",
        success: function (data) {
            if (data.Error != null) {
                ...
                $('#' + _id + ' .fa-bullhorn').parent().toggleClass("active");
                $('#compact' + _id + ' .fa-bullhorn').parent().toggleClass("active");
            }
        },
        error: function (res) {
