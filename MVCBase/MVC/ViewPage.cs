    ...
    public class ViewPage<TModel> : RazorPage<TModel>
    {
        ...
        protected IHtmlContent StaticLayout(IHtmlHelper html, string partialName)
        {
            try
            {
                ...
                var template = _templateService.GetItemByID(_pageEntity.TemplateID);
                if (template == null) return new HtmlString(string.Empty);
                var defaultTemplate = _templateDetailsService.CreateQuery().SelectFirst(o => o.TemplateID == template.ID && o.PartialID == partialName && o.IsDynamic == false);
                if (defaultTemplate == null) return new HtmlString(string.Empty);
                return DynamicViews.RenderWithData(html, defaultTemplate.PartialView, defaultTemplate.CModule, defaultTemplate.GetProperties().ToList());
            }
            catch (Exception ex)
            {
        ...
        protected IHtmlContent DynamicLayout(IHtmlHelper html,string nameLayout)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < limit; i++)
                {
                    ...
                    if (!item.IsBody)
                    {
                        var itemHtml = DynamicViews.RenderWithData(html, item.PartialView, item.CModule, item.GetProperties().ToList());
