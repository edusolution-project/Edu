...
import { INgxSelectOption, INgxSelectOptGroup } from 'ngx-select-ex';
import * as $ from 'jquery';
import { AlertService, MessageSeverity } from 'src/app/services/alert.service';
import { AppComponent } from '../app.component';
import { BsModalService, BsModalRef } from 'ngx-bootstrap/modal';
@Component({
  selector: 'dang-ky-tuyen-sinh-cap2',
  templateUrl: './dang-ky-tuyen-sinh-cap2.component.html'
})
export class DangKyTuyenSinhCap2Component implements OnInit {
  ...
  public lstHocSinh;
  public lstQhuyenTruongCu;
  public lstTruongTieuHoc;
  ...
  flagMaTthanhTruongCu = false;
  flagMaQhuyenTruongCu = false;
  ...
  @ViewChild('tempKetQuaHocTap') ketQuaHocTap: TemplateRef<any>;
  @ViewChild('tempNgoaiNgu1') dangKyNgoaiNgu1: TemplateRef<any>;
  @ViewChild('tempNgoaiNgu2') dangKyNgoaiNgu2: TemplateRef<any>;

  ngOnInit() {

    let arr = ['hoTen', 'noiSinh', 'hoKhauThuongTru', 'hoKhauTamTru', 'tenChuHo', 'songCung', 'soDienThoaiNhaRieng', 'khiCanBaoTin'
      , 'daQuaMauGiao', 'thongTinBo', 'thongTinMe', 'thongTinNguoiNuoiDuong', 'ketQuaHocTap','dangKyNgoaiNgu1','dangKyNgoaiNgu2','chonTruong', 'dangKyHocBanTru', 'tepDinhKem', 'email', 'camKet', 'ghiChu'];
    ...
    this.entity.MaTthanhTruongCu = this.globalServices.soGiaoDucData.MaTinh;
    this.entity.MaQhuyenTruongCu = "";
    ...
    this.quanHuyenService.all("GetByMaTinhThanh").post({ 'MaTinhThanh': this.entity.MaTthanhTruongCu }).subscribe(
      response => {
        this.lstQhuyenTruongCu = this.globalServices.convertObjectToSelect2(response, 'MaQuanHuyen', 'TenQuanHuyen', '--Chọn--');
      }, error => {
        console.log(error);
      }
    );
  ...
  doMaTthanhTruongCu(options: INgxSelectOption[]) {
    let MaTthanhTruongCu: string | number = '-1';
    if (options != undefined && options.length > 0) {
      MaTthanhTruongCu = options[0].value;
    }
    else {
      MaTthanhTruongCu = this.entity.MaTthanhCuTru
    }
    if (this.flagMaTthanhTruongCu == true) {
      this.quanHuyenService.all("GetByMaTinhThanh").post({ 'MaTinhThanh': MaTthanhTruongCu }).subscribe(
        response => {
          this.lstQhuyenTruongCu = this.globalServices.convertObjectToSelect2(response, 'MaQuanHuyen', 'TenQuanHuyen', '--Chọn--');
          if (response.length > 0) {
            $('#dsQhuyenTruongCu .ngx-select').trigger('click');
            setTimeout(() => {
              $('#dsQhuyenTruongCu .ngx-select__choices').addClass('show');
            }, 100);
          }
          else {
            this.entity.MaPxaCuTru = "";
      ...
      this.flagMaTthanhTruongCu = true;
    }
  }


  doMaQhuyenTruongCu(options: INgxSelectOption[]) {
    $('#dsQhuyenTruongCu .ngx-select__choices').removeClass('show');

    let MaQhuyenTruongCu: string | number = '-1';
    if (options != undefined && options.length > 0) {
      MaQhuyenTruongCu = options[0].value;
    }
    else {
      MaQhuyenTruongCu = this.entity.MaQhuyenTruongCu
    }
    if (this.flagMaQhuyenTruongCu == true) {
      this.truongHocService.all("GetByCondition").post({
        'MaTinhThanh': this.entity.MaTthanhTruongCu,
        'MaQuanHuyen': MaQhuyenTruongCu,
        'CapHoc':1
      }).subscribe(
        response => {
          this.lstTruongTieuHoc = this.globalServices.convertObjectToSelect2(response, 'TenTruongHoc', 'TenTruongHoc', '--Chọn--');
          if (response.length > 0) {
            //$('#dsTruongTieuHoc .ngx-select').trigger('click');
            setTimeout(() => {
             // $('#dsTruongTieuHoc .ngx-select__choices').addClass('show');
            }, 100);
          }
          else {
            this.entity.MaPxaCuTru = "";
      ...
      this.flagMaQhuyenTruongCu = true;
    }
  }


  doMaTruongTieuHoc(options: INgxSelectOption[]) {
    $('#dsTruongTieuHoc .ngx-select__choices').removeClass('show');
  }
  ...
  chooseHocSinh(item: dotTuyenSinhdto) {
    ...
    this.entity.MaQhuyenTruongCu = item.MaQhuyenThtru;
    this.entity.MaTthanhTruongCu = item.MaTthanhThtru;
    this.entity.TenTruongCu = item.TenTruongMN;
...
export class dotTuyenSinhdto {
  ...
  NgaySinhBo: any;
    ...
    MaTthanhTmtru: any;
    MaTthonTmtru: string;
    MaTthanhTruongCu: any;
    MaQhuyenTruongCu: string;
